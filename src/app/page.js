    ...
    return (
        <main className="min-h-screen overflow-hidden bg-[#080f1d] text-slate-100">
            ...
            <section className="relative mx-auto min-h-[calc(100vh-60px)] max-w-[1280px] px-6 pb-8 pt-16 sm:px-10 sm:pt-20 lg:px-14">
                ...
                >
                    <Tabs selectedKey={transport} onSelectionChange={key => setTransport(String(key))} className="w-full" variant="underlined">
                        <Tabs.List aria-label="Transport mode" className="grid w-full grid-cols-4 border-b border-white/10">
                            {modes.map(([id, label]) => <Tabs.Tab key={id} id={id} className="justify-center text-[10px] text-slate-400">{label}</Tabs.Tab>)}
                        </Tabs.List>
                    </Tabs>
                    <div className="grid gap-2 p-3 sm:grid-cols-[1.2fr_1.2fr_1fr_72px_80px] sm:items-end">
                        <Select aria-label="From" label="FROM" selectedKey={from} onSelectionChange={key => setFrom(String(key))} size="sm" variant="secondary">
                            <Select.Trigger><Select.Value /></Select.Trigger>
                            <Select.Popover><ListBox>{locations.map(location => <ListBox.Item key={location} id={location}>{location}</ListBox.Item>)}</ListBox></Select.Popover>
                        </Select>
                        <Select aria-label="To" label="TO" selectedKey={to} onSelectionChange={key => setTo(String(key))} size="sm" variant="secondary">
                            <Select.Trigger><Select.Value /></Select.Trigger>
                            <Select.Popover><ListBox>{locations.map(location => <ListBox.Item key={location} id={location}>{location}</ListBox.Item>)}</ListBox></Select.Popover>
                        </Select>
                        <Input aria-label="Date" label="DATE" type="date" value={date} onChange={event => setDate(event.target.value)} size="sm" variant="secondary" />
                        <Select aria-label="Seats" label="SEATS" selectedKey={seats} onSelectionChange={key => setSeats(String(key))} size="sm" variant="secondary">
                            <Select.Trigger><Select.Value /></Select.Trigger>
                            <Select.Popover><ListBox>{[1, 2, 3, 4, 5, 6].map(count => <ListBox.Item key={count} id={String(count)}>{count}</ListBox.Item>)}</ListBox></Select.Popover>
                        </Select>
                        <Button onPress={() => window.alert(`${transport}: ${from} to ${to}`)} className="h-8 bg-[#dd7845] text-[11px] text-white hover:bg-[#ef8a53]" size="sm">
                            <SearchIcon /> Search
                        </Button>
                    </div>
                </div>
