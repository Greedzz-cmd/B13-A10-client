...
function formatTime(value) {
    // Format a ticket departure or arrival timestamp.
...
function formatPrice(price) {
    // Format ticket prices in Bangladeshi taka.
    return `৳${Number(price).toLocaleString("en-IN")}`;
}

function modeIcon(type) {
    // Select the compact icon for a transport type.
    return { Flight: "✈", Train: "▣", Bus: "▧", Launch: "♧" }[type] || "•";
}

function formatDuration(type) {
    // Provide the displayed journey duration for a transport type.
    if (type === "Flight") return "2h 40m";
    if (type === "Launch") return "8h 00m";
    return "6h 40m";
}

function getFareClass(ticket) {
    // Derive the display fare class from ticket price and perks.
...
export default function TicketCard({ ticket }) {
    // Ticket summary card with route, availability, and booking action.
    ...
    return (
        <article className="rounded-lg border border-[#1e3a5f] bg-[#0f172a] p-5 text-slate-100">
            ...
            {/* Departure and arrival route details. */}
            <div className="my-6 grid grid-cols-[1fr_auto_1fr] items-center gap-3">
                ...
                <div className="text-center text-[10px] text-slate-500" aria-hidden="true">
                    <span className="block">•• {formatDuration(ticket.transportType)}</span>
            ...
            {/* Vendor and remaining seat information. */}
            ...
            {/* Rating and seat availability indicator. */}
